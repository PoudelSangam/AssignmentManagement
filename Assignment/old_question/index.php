...
<body class="bg-gray-100">
    ...
    <div class="w-full">
      ...
      <div class="max-w-lg mx-auto mb-4 flex items-center">
        <div class="inline-block flex flex-row w-full">
          <input type="text" class="border rounded-l-lg px-4 py-2 w-full" placeholder="Search Old question" id="search-input">
  ...
  <script>
    ...
    // Function to handle search based on input
    function searchSubjects(query) {
      const searchQuery = query.toLowerCase();
      $('#search-results').empty(); // Clear previous results

      const subjectList = subjects[faculty]?.[curriculumType]?.["Semester " + semester] || [];
      const filteredSubjects = subjectList.filter(subject => subject.toLowerCase().includes(searchQuery));

      if (filteredSubjects.length > 0) {
        filteredSubjects.forEach(subject => {
          $('#search-results').append(`
            <div class="bg-white p-2 hover:bg-gray-100 cursor-pointer">${subject}</div>
          `);
        });
      } else {
        $('#search-results').append('<p class="text-gray-500">No results found.</p>');
      }
    }
