document.addEventListener('DOMContentLoaded', () => {
    ...
    const savedResults = localStorage.getItem('searcholdquestion');
    
  if (savedResults) {
    const parsedResults = JSON.parse(savedResults);
    displaySubjectDetails(parsedResults);
  }
...
function searchSubjects() {
  ...
  localStorage.removeItem('searcholdquestion');
...
function fetchSubjectDetails(subject, faculty, semester) {
  $.ajax({
    ...
    success: function(response) {
      ...
      localStorage.setItem('searcholdquestion', JSON.stringify(response));
      displaySubjectDetails(response);
    },
  ...
  details.forEach(detail => {
    ...
    detailItem.innerHTML = `
      <div class="px-8 py-6 relative">
        <h2 class="font-bold text-2xl mb-4 text-gray-900">${detail.subject}</h2>
     <p class="text-gray-600 mb-4">${detail.faculty} ${detail.batch} </p>
        <span class="absolute top-1 right-4 text-white bg-blue-600 rounded-full px-3 py-1 text-xs font-semibold">${detail.semester}</span>
        
        
        <div class="flex justify-between mt-4">
          <!-- View Button -->
          
          <a href="show_pdf.php?faculty=${encodeURIComponent(detail.faculty)}&semester=${encodeURIComponent(detail.semester)}&subject=${encodeURIComponent(detail.subject)}&id=${encodeURIComponent(detail.pdf_name)}&batch=${encodeURIComponent(detail.batch)}" 
...
<button onclick="handleDownloadClick(this, '${detail.faculty}', '${detail.semester}', '${detail.subject}', '${detail.pdf_name}','${detail.batch}')"
...
function downloadPDF(faculty, semester, subject, pdfName,batch) {
  const downloadUrl = `https://poudelsangam.com.np/Assignment/Notes/file/${encodeURIComponent(batch)}/${encodeURIComponent(faculty)}/${encodeURIComponent(semester)}/${encodeURIComponent(subject)}/${encodeURIComponent(pdfName)}`;
