...
<script>
    ...
    const pdfUrl = `https://poudelsangam.com.np/Assignment/old_question/file/${encodeURIComponent(batch)}/${encodeURIComponent(faculty)}/${encodeURIComponent(semester)}/${encodeURIComponent(pdfName)}`;

 function loadPDFViewer(pdfUrl) {
    // Append a random query string to prevent caching
    const uniqueUrl = pdfUrl + '?_=' + new Date().getTime(); 
    const viewerUrl = 'https://mozilla.github.io/pdf.js/web/viewer.html?file=' + encodeURIComponent(uniqueUrl);
